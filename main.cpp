...
int main(int argc, char* argv[])
{
	...
	ListTyre fixprice;
	cout << "Здесь копия основной базы не создается.\n";
	cout << "вы можете создать/просмотреть список шин с фиксированной ценой(файл price.dat).\n";
	cout << "Введите имя файла, для работы с ценой: ";
	string namepricebd;// строка для названия файла БД
	cin >> namepricebd;

	menu((namepricebd.c_str()), fixprice);  // если файл есть, чтение информации из файла; если нет- создается новый список, отображается меню
						// организует бесконечный цикл, пока не введено значение 6.
